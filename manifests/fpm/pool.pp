define php5::fpm::pool($prefix = '', $listen = '127.0.0.1', $port = '9000',
  ...
  $pm_max_requests = 0, $env = [], $php_values = [], $php_admin_values = [], $access_log = '',
  ...
  $ping_path = '/fpm-ping') {
    ...
    # Make sure directory for access_log exists if access_log is set
    # Will not create the entire directory tree
    if (!empty($access_log)) {
      $access_log_path = regsubst($access_log, '[^\/]*$', '')
      file { "log_path_${name}":
